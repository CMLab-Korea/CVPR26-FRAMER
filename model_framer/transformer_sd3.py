...
class SD3Transformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def forward(
        ...
        for index_block, block in enumerate(self.transformer_blocks):
            ...
            temp_hidden_states, _ = hidden_states.chunk(2, dim=1)
            B, L, C = temp_hidden_states.shape
            temp_hidden_states = self.norm_out(temp_hidden_states, temb)
            temp_hidden_states = self.proj_out(temp_hidden_states)
            temp_hidden_states = temp_hidden_states.reshape(
                shape=(temp_hidden_states.shape[0], height, width, patch_size, patch_size, self.out_channels)
            )
            temp_hidden_states = torch.einsum("nhwpqc->nchpwq", temp_hidden_states)
            temp_output = temp_hidden_states.reshape(
                shape=(temp_hidden_states.shape[0], self.out_channels, height * patch_size, width * patch_size)
            )
            all_hidden_states.append(temp_output)
